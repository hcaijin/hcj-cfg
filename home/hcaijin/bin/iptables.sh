...
finailize()
{
	iptables-save > /etc/iptables/iptables.rules && # 設定の保存
	systemctl reload iptables && # 保存したもので再起動してみる
	return 0
	return 1
}
...
###########################################################
iptables -A INPUT -d 192.168.3.255   -j LOG --log-prefix "drop_broadcast: "
iptables -A INPUT -d 192.168.3.255   -j DROP
